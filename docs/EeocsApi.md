...
------------- | ------------- | -------------
[**eeocsCreate**](EeocsApi.md#eeocsCreate) | **POST** /eeocs | 
...
> EEOC eeocsCreate(xAccountToken, opts)
...
let opts = {
  ...
  'EEOC': new MergeAtsApi.EEOC() // EEOC | 
};
apiInstance.eeocsCreate(xAccountToken, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
