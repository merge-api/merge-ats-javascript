...
> RemoteUser usersCreate(xAccountToken, opts)
...
let opts = {
  ...
  'remoteUserRequest': new MergeAtsApi.RemoteUserRequest() // RemoteUserRequest | 
};
apiInstance.usersCreate(xAccountToken, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
...
let opts = {
  ...
  'email': null, // String | If provided, will only return remote users with the given email address
